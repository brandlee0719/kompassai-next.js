...
export default function CreditDeductedComponent(props) {
  useEffect(() => {
    const chartOptions = {
      chart: {
        type: "area",
      ...
      series: [
        {
          name: "",
          data: [1, 2, 2, 1, 3, 2, 3, 3],
        },
      ],
    };

    const chart = Highcharts.chart("creditDeductedChart", chartOptions);
      ...
      <div className="w-full shrink basis-0 flex-col justify-center items-start gap-4 inline-flex">
        <div className="self-stretch justify-start items-center gap-4 inline-flex">
          ...
          <div className="justify-start items-center gap-4 flex">
            ...
            <div className="text-blue-500 text-base font-normal font-Outfit leading-tight">
              View more
            </div>
          </div>
        </div>
        <div className="text-stone-350 text-sm font-normal font-Outfit leading-tight">
          Total contacts created over time.
        </div>
        <div id="creditDeductedChart" className="w-full "></div>
      </div>
    </div>
  );
}
